...
import { API_V1, type Originator, type Receivable } from '@/lib/types/rails-entities';
import { Field, inputCls } from '@/components/ui/field';
import { SearchSelect } from '@/components/ui/search-select';
import { formatCnpj, formatBrlFromCents } from '@/lib/format';
import { creditOperationStatus, t } from '@/lib/i18n/status';

const STATUSES = Object.keys(creditOperationStatus);
...
export function CreditOperationForm() {
  ...
  const receivables = useAutocomplete<Receivable>(`${API_V1}/receivables`, 'receivables');
  ...
  const receivableOptions = receivables.options.map((r) => ({
    value: String(r.id),
    label: r.reference_number,
    sublabel: formatBrlFromCents(r.amount_cents),
  }));
  ...
  const form = useForm({
    ...
    onSubmit: async ({ value }) => {
      setServerError(null);
      try {
        await postJson(`${API_V1}/credit_operations`, {
          credit_operation: {
            receivable_id: Number(value.receivable_id),
            originator_id: Number(value.originator_id),
            funded_amount_cents: Math.round(parseFloat(value.funded_amount_brl) * 100),
            rate: value.rate,
            status: value.status,
          },
        });
        await queryClient.invalidateQueries({ queryKey: ['credit_operations'] });
        form.reset();
        setOpen(false);
      } catch (e) {
      ...
      {open && (
        ...
        >
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
            <form.Field name="receivable_id" validators={{ onChange: ({ value }) => !value ? 'Obrigatório' : undefined }}>
              {(field) => (
                <Field label="Recebível" error={field.state.meta.errors[0]?.toString()}>
                  <SearchSelect
                    options={receivableOptions}
                    value={field.state.value}
                    onChange={field.handleChange}
                    onQueryChange={receivables.setQuery}
                    isLoading={receivables.isFetching}
            ...
            <form.Field name="originator_id" validators={{ onChange: ({ value }) => !value ? 'Obrigatório' : undefined }}>
              {(field) => (
            ...
            <form.Field name="rate" validators={{ onChange: ({ value }) => !value || isNaN(parseFloat(value)) ? 'Taxa inválida' : undefined }}>
              {(field) => (
                <Field label="Taxa (%)" error={field.state.meta.errors[0]?.toString()}>
                  <input type="number" step="0.01" min="0" className={inputCls} value={field.state.value} onChange={(e) => field.handleChange(e.target.value)} onBlur={field.handleBlur} />
                </Field>
              )}
            </form.Field>
